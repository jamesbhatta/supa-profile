    ...
    <div class="container-fluid">
        ...
        <div class="card z-depth-0">
            ...
            <div class="card-body">
                ...
                <form
                    action="{{ $teacherRatio->id ? route('teacher-ratio.update', $teacherRatio) : route('teacher-ratio.store') }}"
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <select name="district" id="select-district-id" class="custom-select">
                                @isset($teacherRatio->id)
                                    <option value="{{ $teacherRatio->district }}" selected>{{ $teacherRatio ->district }}</option>
                                @else
                    ...
                    <div class="form-group">
                        <button type="submit"
                            class="btn btn-success z-depth-0">{{ $teacherRatio->id ? 'अपडेट गर्नुहोस्' : 'सेभ गर्नुहोस्' }}</button>
                    </div>
                ...
                <table class="table table-striped table-hover">
                    <thead>
                        ...
                        <tr>
                            <th>क्र.स.</th>
                            <th>जिल्लाको</th>
                            <th>आधारभूत तह (१–५)</th>
                            <th>आधारभूत तह (६–८)</th>
                            <th> माध्यमिक तह (९–१०)</th>
                            <th>माध्यमिक तह (११–१२)</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($teacherRatios as $item)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td class="font-roboto">{{ $item->district }}</td>
                                <td class="font-roboto">{{ $item->primary }}</td>
                                <td class="font-roboto">{{ $item->primary_1 }}</td>
                                <td class="font-roboto">{{ $item->secondary }}</td>
                                <td class="font-roboto">{{ $item->secondary_1 }}</td>

                                <td>
                                    <a class="action-btn text-primary" href="{{ route('teacher-ratio.edit', $item) }}"><i
                                            class="far fa-edit"></i></a>
                                    <form action="{{ route('teacher-ratio.destroy', $item) }}" method="post"
