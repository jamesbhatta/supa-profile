...
@section('content')
    <div class="container-fluid">
        <h3 class="font-weight-bold">सरकारी अस्पताल विवरण</h3>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">सामाजिक विकास</li>
                <li class="breadcrumb-item active" aria-current="page">सरकारी अस्पताल विवरण
                </li>
            </ol>
        </nav>
        <div class="container">
        ...
        <div class="card z-depth-0">
            ...
            <div class="card-body">
                ...
                <form
                    action="{{ $govermentHospital->id ? route('goverment-hospital.update', $govermentHospital) : route('goverment-hospital.store') }}"
                    method="POST" class="form">
                    @csrf
                    @if ($govermentHospital->id)
                        @method('put')
                    @endif
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-lg-3">
                            ...
                            <select name="district" id="select-district-id" class="custom-select">
                                @isset($govermentHospital->id)
                                    <option value="{{ $govermentHospital->district }}" selected>{{ $govermentHospital ->district }}</option>
                                @else
                        ...
                        <div class="form-group col-lg-3">
                            <label for="select-district-id">पालिका</label>
                            <select name="municipality" class="custom-select">
                                @isset($govermentHospital->id)
                                    <option value="{{ $govermentHospital->municipality }}" selected>{{ $govermentHospital ->municipality }}</option>
                                @else
                        ...
                        <div class="form-group col-lg-3">
                            <label for="input-fiscal-year-start">अस्पतालको नाम</label>
                            <input type="text" id="input-fiscal-year" name="name" class="form-control font-roboto"
                                value="{{ old('name', $govermentHospital->name) }}">
                        </div>
                    ...
                    <div class="form-group">
                        <button type="submit"
                            class="btn btn-success z-depth-0">{{ $govermentHospital->id ? 'अपडेट गर्नुहोस्' : 'सेभ गर्नुहोस्' }}</button>
                    </div>
        ...
        <div class="card z-depth-0">
            <div class="card-header">
                <h1 class="h3-responsive d-inline-block">सरकारी अस्पताल विवरण</h1>
            ...
            <div class="card-body">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @forelse ($govermentHospitals as $item)
                            <tr>
                                <td class="font-roboto">{{ $item->name }}</td>
                                <td class="font-roboto">{{ $item->district }}</td>
                                <td class="font-roboto">{{ $item->municipality }}</td>

                                <td>
                                    <a class="action-btn text-primary" href="{{ route('goverment-hospital.edit', $item) }}"><i
                                            class="far fa-edit"></i></a>
                                    <form action="{{ route('goverment-hospital.destroy', $item) }}" method="post"
